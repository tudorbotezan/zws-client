...
pub fn Stream(comptime ReaderType: type, comptime WriterType: type) type {
    return struct {
        // Compression is currently disabled for Zig 0.15.2 compatibility
        const DecompressorType = struct {};
        const CompressorType = struct {};
        const Self = @This();
...
pub fn Reader(comptime ReaderType: type) type {
    const BitReader = bit_reader.BitReader(.big, ReaderType);
    return struct {
        ...
        pub fn init(allocator: Allocator, inner_reader: ReaderType, deflate_supported: bool) Self {
            return .{
                .allocator = allocator,
                .bit_reader = bit_reader.bitReader(.big, inner_reader),
...
// Compression test removed for Zig 0.15.2 compatibility
