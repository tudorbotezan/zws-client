    ...
    return struct {
        ...
        pub fn parseResponse(self: *Self) !Response {
            ...
            while (true) {
                ...
                try headers.append(self.arena.allocator(), Header{ .key = key, .value = value });
            }
            ...
            return Response{
                ...
                .headers = try headers.toOwnedSlice(self.arena.allocator()),
            };
        }
    };
}
...
pub const Req = struct {
    ...
    pub fn parse(bytes: []const u8) !struct { Req, usize } {
        ...
        const method = std.meta.stringToEnum(http.Method, method_str) orelse return error.HttpHeadersInvalid;
