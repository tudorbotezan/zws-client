...
test "async" {
    const Handler = struct {
        ...
        pub fn sendZc(self: *Self, buf: []const u8) !void {
            self.sent_frames += 1;
            testing.expectEqual(@as(usize, 6), buf.len) catch unreachable;
            testing.expectEqual(buf[0], 0x8a) catch unreachable;
            testing.expectEqual(buf[1], 0x80) catch unreachable;
